...
pub fn register(gui: &Gui, tx: &Sender<Operation>) {
    ...
    gui.window.connect_configure_event(clone_army!([tx, last_window_size] move |_, configure| on_configure(&tx, configure, last_window_size.clone())));
...
fn on_configure(tx: &Sender<Operation>, configure: &EventConfigure, last_window_size: ArcLastWindowSize) -> bool {
